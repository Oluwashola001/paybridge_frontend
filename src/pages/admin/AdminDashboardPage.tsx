...
const AdminDashboardPage: React.FC = () => {
  ...
  // const [walletAddress, setWalletAddress] = useState(''); // Removed
  // --- ADDED STATES ---
  ...
  // --- UPDATED INVOICE CREATION HANDLER ---
  const handleCreateInvoice = async (e: React.FormEvent) => {
    e.preventDefault();
    setFormError("");
    setNewInvoiceLink("");

    // Updated validation (email/phone are optional)
    if (!clientName || !description || !amount) {
      setFormError("Client Name, Description, and Amount are required.");
      return;
    }
    try {
      // Updated post data
      const response = await axios.post(`${apiUrl}/invoices`, {
        ...
        client_email: clientEmail || null, // Send null if empty
        client_phone: clientPhone || null, // Send null if empty
      });
      if (response.data.success) {
        ...
        setDescription("");
        setAmount("");
        setClientEmail(""); // Reset new field
        setClientPhone(""); // Reset new field
      } else {
  ...
  const handleClearHistory = async () => {
    if (
      window.confirm(
        "ARE YOU ABSOLUTELY SURE you want to delete ALL invoice history? This is irreversible!"
      )
    ) {
      try {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div
          className={`p-6 sm:p-8 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 mb-8 border ${
            isDarkMode
        ...
        >
          ...
          <form onSubmit={handleCreateInvoice} className="space-y-5">
            ...
            {/* --- REPLACED WALLET ADDRESS WITH EMAIL/PHONE --- */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="group">
                <label
                  htmlFor="clientEmail"
            ...
            {formError && (
              <div
                className={`px-4 py-3 rounded-xl text-sm animate-in fade-in duration-200 ${
                  isDarkMode
            ...
            {newInvoiceLink && (
              <div
                className={`px-4 py-4 rounded-xl animate-in slide-in-from-top duration-300 ${
                  isDarkMode
                          ...
                          >
                            ...
                            <td className="px-4 py-4 whitespace-nowrap text-sm font-medium">
                              <div className="flex items-center gap-3">
                                {invoice.status === "PAID" && (
                                  <a
                                    // --- BUG FIX: Replaced localhost with apiUrl ---
                                    href={`${apiUrl}/invoices/${invoice.invoice_id}/receipt`}
                                    target="_blank"
